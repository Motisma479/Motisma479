
# Hi, I'm Mathieu Robion! 👋

<img width="auto" height="175" alt="MonthlyGraph" src="Generated/MonthlyGraphOutput.svg" />

## 🚀 About Me
I've been a developer for 6 years and I have a strong interest in game engines, graphics, and low-level programming.

I enjoy designing systems, experimenting and expanding my knowledge of low-level programming and tools.

## 🛠 Skills
#### <ins>Languages:</ins>
  | C | C++ |
  | :--: | :--: |
  | C# | GLSL |
  | Arduino | Python |
#### <ins>Versioning Tools:</ins>
  | Git      |
  | :------: |
  | Perforce |
#### <ins>IDE / Compiling Tools</ins>
  | Visual Studio      |
  | :----------------: |
  | Visual Studio Code |
  | CMake              |
 
<!-- 
## 🔗 Links
[![ PORTFOLIO](https://img.shields.io/badge/PORTFOLIO-414141?style=for-the-badge&link=https://motisma479.github.io/)](https://motisma479.github.io/)
-->
## ☎️ Contact
| mails: |
| :-------- |
| robion.mathieu.16@gmail.com |

| socials: |
|:-:|
|[Linkedin](www.linkedin.com/in/mathieu-robion-128416266)|
